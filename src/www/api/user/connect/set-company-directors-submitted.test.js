...
describe('/api/user/connect/set-company-directors-submitted', function () {
  ...
  describe('exceptions', () => {
    ...
    describe('invalid-stripe-account', () => {
      it('ineligible stripe account for individual', async () => {
        ...
        const req = TestHelper.createRequest(`/api/user/connect/set-company-directors-submitted?stripeid=${user.stripeAccount.stripeid}`)
        req.account = user.account
        req.session = user.session
        let errorMessage
        try {
  ...
  describe('returns', () => {
    it('object', async () => {
      ...
      assert.strictEqual(accountNow.stripeObject.company.directors_provided, true)
    })
  })
})
