...
describe('/account/connect/person', function () {
  describe('before', () => {
    ...
    it('should bind data to req', async () => {
      ...
      const req = TestHelper.createRequest(`/account/connect/person?personid=${user.owner.personid}`)
      req.account = user.account
      req.session = user.session
      await req.route.api.before(req)
  ...
  describe('view', () => {
    ...
    it('should show person is director', async () => {
      ...
      const req = TestHelper.createRequest(`/account/connect/person?personid=${user.director.personid}`)
      req.account = user.account
      req.session = user.session
      const result = await req.get()
      const doc = TestHelper.extractDoc(result.html)
