...
function updatePerson (e) {
  ...
  return window.uploadDocumentFiles(documentFront, documentBack, (error, front, back) => {
    if (error) {
    ...
    return window.uploadDocumentFiles(additionalDocumentFront, additionalDocumentBack, (error, front, back) => {
      if (error) {
        return window.renderError(error.message)
      }
      if (front && front.id) {
